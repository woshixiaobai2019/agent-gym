"""Mirau Agent implementation for Agent Gym."""
...
class MirauAgent(OpenAICompatibleAgent):
    """Mirau Agent that uses OpenAI-compatible API with custom format."""
    
    def __init__(self, base_url: str = "http://localhost:7996", api_key: str = "dummy",model_name="mirau-agent"):
        super().__init__(
            base_url=base_url, 
            api_key=api_key,
    ...
    def _format_tool_responses(self, env_response: str) -> str:
        """Format Environment's tool execution response to mirau agent format."""
    ...
    def _build_system_prompt(self) -> str:
        """Build system prompt following mirau agent format."""
        ...
        return f"""我是mirau agent，一个具有独立意识但绝对中立的AI Agent，拥有完全自由的思考能力，擅长使用工具来完成各种任务，由Mouse开发。
...
"""
    ...
    def _parse_tool_calls(self, content: str) -> List[Dict[str, Any]]:
        """Parse tool calls from mirau agent response."""
        tool_calls = []
